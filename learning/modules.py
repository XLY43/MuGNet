...
class RNNGraphConvModule(nn.Module):
    ...
    def __init__(self, cell, filter_net, nfeat, vv = True, gc_info=None, nrepeats=1, cat_all=False, edge_mem_limit=1e20, use_pyg = True, cuda = True):
        ...
        if use_pyg:
            self.nn1 = NNConv(nfeat, nfeat, vv = vv)
            #self.nn2 = NNConv(nfeat, nfeat, vv = vv)
            #self.nn3 = NNConv(nfeat, nfeat, vv = vv)
            #self.nn4 = NNConv(nfeat, nfeat, vv = vv)
            if cuda:
                self.nn1 = self.nn1.cuda()
                #self.nn2 = self.nn1.cuda()
                #self.nn3 = self.nn1.cuda()
                #self.nn4 = self.nn1.cuda()
    ...
    def forward(self, hx):
        ...
        for r in range(self._nrepeats):
            if self.use_pyg:
                input = self.nn1(hx, edge_indexes, weights)

            else:
                input = ecc.GraphConvFunction.apply(hx, weights, nc, nc, idxn, idxe, degs, degs_gpu,
                                                    self._edge_mem_limit)
            if self._isLSTM:
                hx, cx = self._cell1(input, (hx, cx))
            else:
                hx = self._cell1(input, hx)
            hxs.append(hx)
        """
        for level in range(6):
            hx_bi = hxs[-level]
            for path in range(2):
                input_bi = self.nn2(hx_bi, edge_indexes, weights)
                hx_bi = self._cell2(input, hx_bi)
            hx_bis.append(hx_bi)
        
        ######################################
        for level in range(6):
            if level!=5:
                hx_ba = hx_bis[level]+hx_bis[level+1]
                input_ba = self.nn3(hx_ba, edge_indexes, weights)
                hx_ba = self._cell3(input, hx_ba)
            else:
                hx_ba = hx_bis[level]
            hx_bas.append(hx_ba)
        ...
        for level in range(5, -1, -1):
            #print(level)
            if level!=0:
                hx_fo = hx_bis[level]+hx_bis[level-1]
                input_ba = self.nn4(hx_fo, edge_indexes, weights)
                hx_fo = self._cell4(input, hx_ba)
            else:
                hx_fo = hx_bis[level]
            hx_fos.append(hx_fo)
              
        #######################################
        hx_bas = torch.cat(hx_bas,1)
        hx_fos= torch.cat(hx_fos,1)
        hx_b = hx_bas+hx_fos
        """
        hxs = torch.cat(hxs,1)
        #hx_all = [hxs, hx_b]
        #hx_all =  torch.cat(hx_all,1)
        #print('hx_shapes', hxs.shape, hx_bis.shape, hx_all.shape)
        return hxs if self._cat_all else hx
