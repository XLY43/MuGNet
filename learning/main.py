...
from learning import multires as pointnet
from learning import metrics
from learning import splineconv
...
def main():
    ...
    parser.add_argument('--lr', default=8e-3, type=float, help='Initial learning rate')
    parser.add_argument('--lr_decay', default=0.9, type=float, help='Multiplicative factor used on learning rate at `lr_steps`')
    ...
    #print("valid_dataset", valid_dataset[1])
    ...
    #print("Train dataset element1", train_dataset[0][0].shape)
    #print("Train dataset element2", train_dataset[0][1].get_edgelist())
    #print("Train dataset element3", len(train_dataset[0][2]))
    #print("Train dataset element4", len(train_dataset[0][3]))
    #print("Train dataset element5", train_dataset[0][4].shape)
    ...
    def train():
        ...
        for bidx, (targets, GIs, clouds_data) in enumerate(loader):
            ...
            # label_mode is the tensor that records labels for all clouds [num_clouds]
            # print("label_mode", label_mode.shape)
            # [num_clouds, 13]
            # print("label_vec", label_vec.shape)
            # [num_clouds]
            # print("segm_size", segm_size.shape)
            ...
            """
            edge_attr = GIs[0]._edgefeats
            min_v = torch.min(edge_attr)
            range_v = torch.max(edge_attr) - min_v
            if range_v > 0:
                normalised = (edge_attr - min_v) / range_v
            else:
                normalised = torch.zeros(edge_attr.size())
            edge_attr = normalised[:,:5]
            """
            edge_idx = GIs[0]._edge_indexes
            edge_attr = get_eattr(GIs[0]._edgefeats)
            embeddings, edge_idx, edge_attr = embeddings.cuda(), edge_idx.cuda(), edge_attr.cuda()
            outputs = model.ecc(embeddings, GIs[0]._edge_indexes, edge_attr)
            ...
            if args.grad_clip>0:
                for p in model.parameters():
                    #print(p.grad)
                    p.grad.data.clamp_(-args.grad_clip, args.grad_clip) #>>>>>>>>>>>>>>>error here 'NoneType' object has no attribute 'data'
    ...
    def eval(is_valid = False):
        ...
        if is_valid: #validation
            loader = torch.utils.data.DataLoader(valid_dataset, batch_size=1, collate_fn=spg.eccpc_collate, num_workers=args.nworkers, shuffle=True, drop_last=True)
        else: #evaluation
            loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, collate_fn=spg.eccpc_collate, num_workers=args.nworkers)
        #print("check length valid set", len(valid_dataset), len(loader))
        if logging.getLogger().getEffectiveLevel() > logging.DEBUG: loader = tqdm(loader, ncols=65)
        with torch.no_grad():
            ...
            loss_meter = tnt.meter.AverageValueMeter()
            confusion_matrix = metrics.ConfusionMatrix(dbinfo['classes'])
            #print("check eval loop", len(loader))
            # iterate over dataset in batches
            for bidx, (targets, GIs, clouds_data) in enumerate(loader):
                #print("check loader loop")
                model.ecc.set_info(GIs, args.cuda)
                #print('length of gc_infos', len(GIs))
                ...
                ###########normalize edge features
                # ecc is replaced by splineconv
                #outputs = model.scnn(embeddings, GIs[0]._edge_indexes, edge_attr)
                edge_idx = GIs[0]._edge_indexes
                edge_attr = get_eattr(GIs[0]._edgefeats)
                embeddings, edge_idx, edge_attr = embeddings.cuda(), edge_idx.cuda(), edge_attr.cuda()
                outputs = model.ecc(embeddings, GIs[0]._edge_indexes, edge_attr)
                #if len(clouds_data[0])>16000:
                #    continue
                outputs = model.ecc(embeddings, GIs[0]._edge_indexes, edge_attr)
    ...
    def eval_final():
        ...
        with torch.no_grad():
            ...
            for ss in range(2):
                ...
                loader = torch.utils.data.DataLoader(test_dataset_ss, batch_size=18, collate_fn=spg.eccpc_collate, num_workers=args.nworkers)
                if logging.getLogger().getEffectiveLevel() > logging.DEBUG: loader = tqdm(loader, ncols=65)
...
def create_model(args, dbinfo):
    ...
    #model.scnn = splineconv.splineconv()
...
def get_eattr(edge_feats, num_keep=6):
    ...
    if range_v > 0:
        ...
        normalised = torch.zeros(edge_attr.size())
    edge_attr = normalised[:,:]
    return edge_attr

# scnn(embeddings, GIs[0]._edge_indexes, edge_attr)
"""
class new_graph(Dataset):    
    def __init__(self, embeddings, GIs):
        self.embeddings
        self.GIs = GIs
    def __len__(self):
        return len(self.GIs)
    def __getitem__(self, index):
        edge_indexes = GIs[index]._edge_indexes
        edge_attr = get_edge_attr(GIs[index]._edgefeats)
        return embeddings[index], edge_indexes, edge_attr
"""
